...
<table width="654" align="center" cellpadding="2" class="bordes_tablas">
  <tr>
    <th colspan="2" bgcolor="#FFFFFF" scope="col"><span class="Estilo14">Visualizar Almacen</span></th>
  </tr>
  <tr>
    <th width="535" bgcolor="#FFCC66" scope="col">Consumible</th>
    <th width="97" bgcolor="#FFCC66" scope="col">Diponibles</th>
  </tr>
  <?php do { 
$query_almacen = "SELECT sum(cantidad) FROM almacen where consumible='$row_consumibles[id_copnsumible]'";
...
$disponible=$row_almacen["sum(cantidad)"]-$row_pedido["sum(cantidad)"];
...
<table border="0" align="center">
  <tr>
    <td><?php if ($pageNum_consumibles > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_consumibles=%d%s", $currentPage, 0, $queryString_consumibles); ?>">Primero</a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_consumibles > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_consumibles=%d%s", $currentPage, max(0, $pageNum_consumibles - 1), $queryString_consumibles); ?>">Anterior</a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_consumibles < $totalPages_consumibles) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_consumibles=%d%s", $currentPage, min($totalPages_consumibles, $pageNum_consumibles + 1), $queryString_consumibles); ?>">Siguiente</a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_consumibles < $totalPages_consumibles) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_consumibles=%d%s", $currentPage, $totalPages_consumibles, $queryString_consumibles); ?>">&Uacute;ltimo</a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
</body>
